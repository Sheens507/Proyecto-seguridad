...
/*Procedimientos*/
/*Procedimiento para insertar tipo de habitaciones*/
DELIMITER //
...
/*Procedimiento para asignar habitaciones*/
DELIMITER //
CREATE PROCEDURE Asignar(
IN ID_reserva INT,
IN ID_hab INT
)
BEGIN
...
/*Procedimiento para ver disponibilidad de habitaciones*/
DELIMITER //
...
/*Procedimiento para insertar tarjeta para el pago*/
DELIMITER //
...
FROM 
    Cliente C
JOIN 
    Reservacion R ON C.ID_Cliente = R.ID_cliente_FK
JOIN 
    Asignacion ON Asignacion.ID_reserv_FK = R.ID_reserv
JOIN 
    Habitacion H ON H.ID_hab = Asignacion.ID_hab_FK
JOIN 
    Tipo_habitacion T ON T.ID_tipo = H.ID_tipo_hab
ORDER BY 
    C.Nombre_cli;
